...
import { SCHEMA, log } from '@mountnotion/utils';
...
export const createDatabaseSchema = async ({
  selectedPages,
}: MountNotionConfig): Promise<Array<Schema>> => {
  ...
  const [page] = flattenPageResponse<Schema>(pageResponse);
  ...
  const schema: Schema[] = allUsableDatabases
    .map((database) => {
    ...
    .map((scheme, i, schema) => {
      ...
      return {
        ...
        relations: createRelations(allUsableDatabases[i].properties, schema),
      };
    });
