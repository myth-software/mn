...
horsesRouter.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const newHorses = await mn.horses.create({ name, ...item });
...
import { indicies } from '@pferd/domain-pferd-entities';
export const mn =
  process.env.NODE_ENV === 'production'
    ? client
    : configure({ integrationKey: process.env.INTEGRATION_KEY, indicies });
```
